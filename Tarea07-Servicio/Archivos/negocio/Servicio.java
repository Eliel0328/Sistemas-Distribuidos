...
public class Servicio {
    ...
    public Response alta(@FormParam("usuario") Usuario usuario) throws Exception {
        ...
        try {
            ...
            PreparedStatement stmt_4 = conexion
                    .prepareStatement("select * from usuarios order by id_usuario desc limit 1");
            try {
                ResultSet rs = stmt_4.executeQuery();
                try {
                    if (rs.next()) {
                        Usuario u = new Usuario();
                        u.id_usuario = rs.getInt(1);
                        return Response.ok().entity(j.toJson(u.id_usuario)).build();
                    }
    ...
    public Response consulta(@FormParam("id_usuario") int id_usuario) throws Exception {
        ...
        try {
            PreparedStatement stmt_1 = conexion.prepareStatement(
                    "SELECT id_usuario,email,nombre,apellido_paterno,apellido_materno,fecha_nacimiento,telefono,genero FROM usuarios WHERE id_usuario=?");
            try {
    ...
    public Response modifica(@FormParam("usuario") Usuario usuario) throws Exception {
        ...
        try {
            ...
            PreparedStatement stmt_1 = conexion.prepareStatement(
                    "UPDATE usuarios SET email=?,nombre=?,apellido_paterno=?,apellido_materno=?,fecha_nacimiento=?,telefono=?,genero=? WHERE id_usuario=?");
            try {
                stmt_1.setString(1, usuario.email);
            ...
            return Response.status(401).entity(j.toJson(new Error(e.getMessage()))).build();
        } finally {
            conexion.close();
        }
        return Response.ok().build();
